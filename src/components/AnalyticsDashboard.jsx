...
function StatCard({ icon: Icon, label, value, sub, color }) {
  return (
    <div className="bg-[var(--bg-surface)] rounded-2xl border border-border dark:border-slate-800 p-5 shadow-sm">
...
function ThetaDistribution({ students }) {
  ...
  return (
    <div className="bg-[var(--bg-surface)] rounded-2xl border border-border dark:border-slate-800 p-6 shadow-sm">
      <h3 className="text-sm font-black text-foreground uppercase tracking-wider mb-1 flex items-center gap-2">
      ...
      <div className="space-y-3">
        {distribution.map((d, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-[10px] font-bold text-secondary w-28 text-right shrink-0">{d.label}</span>
  ...
  return (
    <div className="bg-[var(--bg-surface)] rounded-2xl border border-border dark:border-slate-800 p-6 shadow-sm">
      ...
      {students && students.length > 0 && (
        <div className="mt-5">
          <p className="text-[10px] font-bold text-secondary uppercase tracking-wider mb-3">Detail Per Mahasiswa</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900 border-b border-border dark:border-slate-800">
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-secondary uppercase">Nama</th>
                  <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Pre</th>
                  <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Post</th>
                  <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">N-Gain</th>
                  <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Kategori</th>
                </tr>
              </thead>
              <tbody>
                {students.map((s, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-border dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                      <td className="px-4 py-2.5 font-bold text-foreground text-xs">{s.student_name}</td>
                      <td className="text-center px-3 py-2.5 text-xs text-secondary">{s.pre_score}</td>
                      <td className="text-center px-3 py-2.5 text-xs text-secondary">{s.post_score}</td>
                      <td className="text-center px-3 py-2.5 text-xs font-bold text-foreground">{s.gain?.toFixed(3)}</td>
    ...
    <div className="bg-[var(--bg-surface)] rounded-2xl border border-border dark:border-slate-800 p-6 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-border dark:border-slate-800">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-border dark:border-slate-800">
              <th className="text-left px-4 py-2.5 text-[10px] font-bold text-secondary uppercase">Mahasiswa</th>
              <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">θ (Theta)</th>
              <th className="text-left px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Kategori</th>
              <th className="text-center px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Respons</th>
              <th className="px-3 py-2.5 text-[10px] font-bold text-secondary uppercase">Visual</th>
            </tr>
          </thead>
          <tbody>
            {students.map((s, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                  <td className="px-4 py-2.5 font-bold text-foreground text-xs">{s.full_name}</td>
                  <td className="text-center px-3 py-2.5 text-xs font-mono font-bold text-foreground">{s.theta?.toFixed(3)}</td>
                  <td className="px-3 py-2.5">
                    <span className="text-xs font-bold" style={{ color: s.category_color }}>{s.category_label}</span>
                  </td>
                  <td className="text-center px-3 py-2.5 text-xs text-secondary">{s.responses_count || 0}</td>
                  <td className="px-3 py-2.5">
    ...
    return (
      <div className="h-full flex items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="text-center">
          ...
          <button onClick={fetchAnalytics} className="px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary-hover transition">
            Coba Lagi
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-secondary mt-1">Dashboard analitik psikometrik untuk room ini.</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={exportPDF}
              aria-label="Export laporan analitik ke PDF"
              className="flex items-center gap-2 px-4 py-2.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-sm font-bold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition shadow-sm"
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-xl text-sm font-bold text-indigo-700 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition shadow-sm"
              disabled={irtStudents.length === 0}
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-[var(--bg-surface)] border border-border dark:border-slate-800 rounded-xl text-sm font-bold text-secondary hover:bg-slate-50 dark:hover:bg-slate-800 transition shadow-sm"
            >
        ...
        {irtStudents.length > 0 && (
          <div className="bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl border border-indigo-100 dark:border-indigo-500/20 p-6 shadow-sm mb-8 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <Bot size={100} className="text-indigo-900" />
            </div>
            <h3 className="text-sm font-black text-indigo-900 dark:text-indigo-400 uppercase tracking-wider mb-3 flex items-center gap-2 relative z-10">
              <Bot size={18} className="text-primary" /> AI Insight & Rekomendasi
            </h3>
            <div className="relative z-10 text-sm text-indigo-900 dark:text-indigo-200 leading-relaxed whitespace-pre-wrap">
              {isAiLoading ? (
                ...
                <div className="bg-[var(--bg-surface)] shadow-sm border border-indigo-200 dark:border-indigo-500/30 p-4 rounded-xl text-indigo-700 dark:text-indigo-400 text-[13px]">
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-center text-[10px] text-secondary mt-2 font-medium">
              *<strong className="text-secondary">θ (Theta)</strong> adalah estimasi kemampuan laten mahasiswa menurut IRT Rasch Model. Semakin besar θ, semakin tinggi pemahaman konsepnya.
            </p>
          </div>
        ...
        {/* No data notice */}
        {irtStudents.length === 0 && (
          <div className="text-center py-20 bg-[var(--bg-surface)] rounded-2xl border border-dashed border-border dark:border-slate-700 mt-6">
