...
export default function MobileBottomNav({ activeTab, onTabChange, coinCount }) {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 bg-[var(--bg-surface)] dark:bg-[var(--bg-surface)] border-t border-border dark:border-slate-800 dark:border-slate-800 safe-area-bottom transition-colors"
        ...
        {NAV_ITEMS.map(({ id, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-semibold leading-none ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-secondary dark:text-slate-400'}`}>
                {label}
              </span>
            </button>
          );
