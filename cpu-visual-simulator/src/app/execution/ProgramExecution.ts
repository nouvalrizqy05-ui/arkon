...
import { sendToParent } from "../util/bridge"
...
export default class ProgramExecution implements Cyclable {
	...
	public reset(): void {
		const wasHalting = this.executionContext.cpu.model.isHalting.get()
		this.pause()
		if (wasHalting) {
			this.sendExecutionUpdate("HALTED", { message: "Simulasi telah selesai." })
		}
		this.executionContext.cpu.model.isHalting.set(false)
		this.executionContext.cpu.model.isJumping.set(false)
		this.cyclePhase = "ENQUEUING_FETCH"
		this.emptyExecutionQueue()
		if (this.executionContext.stepTextComponent) {
			this.executionContext.stepTextComponent.setText("")
		}
		try {
			// cancel any playing text to speech
	...
	protected sendExecutionUpdate(status: string, details: Record<string, unknown> = {}): void {
		const cpuModel = this.executionContext.cpu.model
		const instruction = cpuModel.instructionRegister.get()
		const payload = {
			status,
			data: {
				pc: cpuModel.programCounter.get().unsigned?.(),
				acc: cpuModel.accumulator.get().unsigned?.(),
				instruction: typeof instruction === "string" ? instruction : instruction.symbolic?.(),
				isHalting: cpuModel.isHalting.get(),
				isJumping: cpuModel.isJumping.get(),
				...details
			}
		}
		sendToParent("ARKON_SIMULATION_RESULT", payload)
	}
