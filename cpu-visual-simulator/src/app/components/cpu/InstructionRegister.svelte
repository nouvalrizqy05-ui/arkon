...
>
	...
	{#if !$instructionRegister.opcode && !$displayAsBinary}
		<div class="w-[200px] h-[30px] leading-[30px] col-span-2 text-center rounded-md text-indigo-light"
			style="background: #1e293b;">
			{$text.cpu.ir.invalid_instruction}
		</div>
	{:else}
		<div class="w-full h-[28px] leading-[30px] text-center rounded-l-md text-pink-light" style="background: #1e293b;" bind:this={opcodeDiv}>
			{opcode}
		</div>
		<div class="w-full h-[28px] leading-[30px] text-center rounded-r-md text-indigo-light" style="background: #1e293b;" bind:this={operandDiv}>
			{operand}
		</div>
	{/if}
</div>
