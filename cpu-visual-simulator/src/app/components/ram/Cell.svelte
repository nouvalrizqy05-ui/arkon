...
<div
	class="
		...
		{$$restProps.class}
	"
	style="background: {isActive ? 'rgb(16, 185, 129)' : '#1e293b'}; border-color: rgba(16, 185, 129, 0.2);"
	on:click={select}
	bind:this={cell}
>
	{#if isSelected}
		<input
			...
			class="
				...
				font-mono
			"
			style="background: #0f172a; color: #ec4899;"
		/>
	{:else}
		<div class="h-[30px] w-[95px] leading-[30px] text-left pl-[10px] text-pink-light">{opcode}</div>
		{#if operand}
			<div
				class="h-[30px] w-[95px] min-w-0 leading-[30px] text-left overflow-hidden text-ellipsis whitespace-nowrap text-indigo-light"
			>
				{operand}
			</div>
		{/if}
