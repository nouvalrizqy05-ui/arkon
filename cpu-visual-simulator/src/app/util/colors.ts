...
const RGBA_PATTERN = /rgba\(\s*\d{1,3}\s*\,\s*\d{1,3}\s*\,\s*\d{1,3}\s*\,\s*\d+(?:\.\d+)?\s*\)/
...
export class Color {
	...
	public static readonly GREEN = new Color(16, 185, 129)
}
...
export function rgbaStringToColor(rgba: string): Color {
	...
	const a = matches[3] !== undefined ? parseFloat(matches[3]) : 255
